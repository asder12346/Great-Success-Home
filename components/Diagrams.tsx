...
// --- EVENT CARD ---
export const EventCard = ({ title, date, location, description, image, onClick }: any) => {
    return (
        <div 
            className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 flex flex-col h-full group cursor-pointer"
            onClick={onClick}
        >
            ...
            <div className="p-8 flex flex-col flex-grow">
                <div className="flex items-center gap-2 text-slate-400 text-xs font-bold uppercase tracking-widest mb-3">
                    <MapPin size={14} className="text-gs-gold" />
                    {location}
                </div>
                <h3 className="font-serif text-2xl font-bold text-slate-900 mb-4 group-hover:text-gs-green transition-colors">
                    {title}
                </h3>
                ...
                <div className="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between">
                    <span className="text-gs-green font-bold text-sm flex items-center gap-2">
...
// --- EVENTS CAROUSEL ---
export const EventsCarousel = ({ events }: { events: any[] }) => {
    const [currentIndex, setCurrentIndex] = useState(0);
    ...
    return (
        <div className="relative w-full">
            ...
            <div className="flex justify-center gap-6 mt-12">
                <button 
                    onClick={prev} 
                    className="w-14 h-14 rounded-full border-2 border-slate-200 flex items-center justify-center text-slate-400 hover:bg-gs-green hover:border-gs-green hover:text-white transition-all transform hover:-translate-x-1 active:scale-95 bg-white shadow-sm"
                >
                    <ChevronLeft size={24} />
                </button>
                <div className="flex items-center gap-3">
                    {Array.from({ length: maxIndex + 1 }).map((_, idx) => (
                        <button 
                            key={idx}
                            onClick={() => setCurrentIndex(idx)}
                            className={`transition-all duration-300 rounded-full ${idx === currentIndex ? 'w-10 h-2 bg-gs-green' : 'w-2 h-2 bg-slate-300 hover:bg-gs-gold'}`}
                        />
                    ))}
                </div>
                <button 
